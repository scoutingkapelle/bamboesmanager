...
case class DBRegistration(id: UUID, person_id: UUID, friday: Boolean, saturday: Boolean, sorting: Boolean, category_id: UUID)

class RegistrationTable(tag: Tag) extends Table[DBRegistration](tag, "registrations") {
  val categories = TableQuery[CategoryTable]
  val persons = TableQuery[PersonTable]

  def * = (id, person_id, friday, saturday, sorting, category_id) <>(DBRegistration.tupled, DBRegistration.unapply)
